...
#     return torch.nn.BatchNorm2d(in_features, eps=eps, momentum=momentum, affine=affine, track_running_stats=True)

class ConditionalBN(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features):
        ...
        self.gain = torch.nn.Linear(in_features=128, out_features=out_features, bias=False)
        self.bias = torch.nn.Linear(in_features=128, out_features=out_features, bias=False)

    def forward(self, x, y):
        #print("y=",y.shape) #y= torch.Size([1, 8])
...
class BasicResBlock(nn.Module):
    def __init__(self, inplanes, outplanes, norm_layer=nn.BatchNorm2d, nl_layer=nn.ReLU):
        super(BasicResBlock, self).__init__()

        # 主路径
        self.conv = nn.Sequential(
            norm_layer(inplanes) if norm_layer is not None else nn.Identity(),
            nl_layer(),
            nn.Conv2d(inplanes, outplanes, kernel_size=3, stride=1, padding=1, bias=False),
            norm_layer(outplanes) if norm_layer is not None else nn.Identity(),
            nl_layer(),
...
class G_ResNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, n_blocks=3, norm_layer=nn.BatchNorm2d, nl_layer=nn.ReLU):
        super(G_ResNet, self).__init__()

        # 初始卷积层
        self.initial_conv = nn.Conv2d(input_nc, ngf, kernel_size=3, stride=1, padding=1, bias=True)

        # 残差块堆叠
        self.res_blocks = nn.Sequential(*[BasicResBlock(ngf, ngf, norm_layer, nl_layer) for _ in range(n_blocks)])

        # 最终卷积层，调整到输出通道数
        self.final_conv = nn.Conv2d(ngf, output_nc, kernel_size=3, stride=1, padding=1, bias=True)
...
#             nn.Conv2d(channels * 2, channels // 8, kernel_size=1, stride=1, padding=0),  # 降维
#             nn.BatchNorm2d(channels // 8),
#             nn.ReLU(inplace=True),
#             nn.Conv2d(channels // 8, 1, kernel_size=1, stride=1, padding=0),  # 生成注意力图
#             nn.Sigmoid()  # 归一化到 [0, 1]
#         )
#
#         # # 2. 特征融合
#         # self.fusion_conv = nn.Sequential(
#         #     nn.Conv2d(channels * 2, channels, kernel_size=3, stride=1, padding=1),  # 融合特征
...
#         """
#         Args:
#             f1: 来自UNet的特征图 (B, C, H, W)
#             f2: 来自残差块的特征图 (B, C, H, W)
#         Returns:
#             融合后的特征图 (B, C, H, W)
#         """
#         # 拼接特征图
#         combined = torch.cat([f1, f2], dim=1)  # (B, 2C, H, W)
#
#         # 生成注意力权重
#         attention_map = self.attention(combined)  # (B, 1, H, W)
#
#         # # 使用注意力权重加权融合
#         weighted_f2 = f2 * attention_map
#         # weighted_f2 = f2 * (1 - attention_map)  # 加权f2
#         #
#         # # 拼接加权后的特征图
#         fused = torch.cat([f1, weighted_f2], dim=1)  # (B, 2C, H, W)
#         #
#         # # 通过卷积进一步融合
...
class AttentionModule(nn.Module):
    def __init__(self, in_channels, reduction_ratio=8):
        ...
        self.spatial_attention = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // reduction_ratio, 1, 3, padding=1),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, f1, f2):
        """
        Args:
            f1: UNet输出的特征图 [B, C, H, W]
            f2: 残差网络输出的特征图 [B, C, H, W]
        Returns:
            增强后的特征图
        """
        # 特征拼接融合
        fused = self.feature_fusion(torch.cat([f1, f2], dim=1))
        ...
        # 组合注意力
        combined_attention = channel_weights * spatial_weights

        # 应用注意力机制

        enhanced = torch.cat([f1, combined_attention * f2], dim=1)
...
class G_Unet_add_all(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
        ...
        unet_block = UnetBlock_with_z(input_nc, output_nc, ngf, nz, unet_block,
                                      outermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        # self.model = unet_block
        self.mappnet =  MappingNetwork(z_dim=nz,w_dim=128)
        self.umodel = unet_block
        self.rmodel = G_ResNet(input_nc, output_nc, ngf, n_blocks=4, norm_layer=nn.BatchNorm2d, nl_layer=nn.ReLU)
        self.att = AttentionModule(64)
    ...
    def forward(self, x, z):
        ...
        f2 = self.rmodel(x)
        # print(f2.size()) 
        f_fusion = self.att(f1, f2)
        # print(f_fusion.size())  #[4,64,256,256]
        output = self.final_conv(f_fusion)
        return output
...
class UnetBlock_with_z(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc, nz=0,
        ...
        self.cbn1 = ConditionalBN(in_features=inner_nc,out_features=inner_nc)
        self.cbn2 = ConditionalBN(in_features=outer_nc, out_features=outer_nc)
